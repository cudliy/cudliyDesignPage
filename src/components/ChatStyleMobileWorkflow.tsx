...
import { isMobileDevice, validateMobileStorage, compressImageForMobile } from '../utils/mobileUtils';
...
export default function ChatStyleMobileWorkflow({ onError }: ChatStyleMobileWorkflowProps) {
  ...
  const handleFileUpload = async (files: FileList) => {
    ...
    try {
      ...
      const processedFiles = isMobileDevice() 
        ? await Promise.all(validFiles.map(file => compressImageForMobile(file)))
        : validFiles;
      ...
      const message = isMobileDevice() 
        ? `${validFiles.length} image(s) uploaded and optimized for mobile`
  ...
  const handleImageShare = async (selectedImages: UploadedImage[]) => {
    ...
    try {
      ...
      try {
        ...
        // Use mobile-specific storage validation
        const storageValidation = validateMobileStorage(shareDataString.length);
        if (!storageValidation.isValid) {
          throw new Error(storageValidation.message || 'Storage limit exceeded');
        }
                      ...
                      {message.images.map((imageUrl, index) => (
                        ...
                        >
                          ...
                          <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-200">
                            <button className="px-6 h-[54px] bg-[#313131] text-white rounded-full font-medium text-sm shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200 flex items-center justify-center">
                              View 360°
                            </button>
                          </div>
                        </div>
